<p align="center">
  <img src=".github/assets/icon-dark.webp" alt="Vox" width="120" height="120" />
</p>
...
<p align="center">
  <a href="https://github.com/Clawbotsai/vox/releases">
    <img src="https://img.shields.io/github/downloads/Clawbotsai/vox/total?style=flat&color=blue" alt="Downloads" />
  </a>
  <a href="https://github.com/Clawbotsai/vox/releases/latest">
    <img src="https://img.shields.io/github/v/release/Clawbotsai/vox?style=flat" alt="Release" />
  </a>
  <a href="https://github.com/Clawbotsai/vox/stargazers">
    <img src="https://img.shields.io/github/stars/Clawbotsai/vox?style=flat" alt="Stars" />
  </a>
  <a href="https://github.com/Clawbotsai/vox/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/Clawbotsai/vox?style=flat" alt="License" />
  </a>
  <a href="https://deepwiki.com/Clawbotsai/vox">
    <img src="https://img.shields.io/static/v1?label=Ask&message=DeepWiki&color=5B6EF7" alt="Ask DeepWiki" />
  </a>
</p>

<p align="center">
    <a href="https://trendshift.io/repositories/21213" target="_blank"><img src="https://trendshift.io/api/badge/repositories/21213" alt="jamiepine%2Fvox | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>
</p>

<p align="center">
  <a href="https://vox.sh">vox.sh</a> •
  <a href="https://docs.vox.sh">Docs</a> •
...
<p align="center">
  <a href="https://vox.sh">
    <img src="landing/public/assets/app-screenshot-1.webp" alt="Vox App Screenshot" width="800" />
  </a>
</p>

<p align="center">
  <em>Click the image above to watch the demo video on <a href="https://vox.sh">vox.sh</a></em>
</p>

<br/>

<p align="center">
  <img src="landing/public/assets/app-screenshot-2.webp" alt="Vox Screenshot 2" width="800" />
</p>

<p align="center">
  <img src="landing/public/assets/app-screenshot-3.webp" alt="Vox Screenshot 3" width="800" />
</p>
...
Vox is a **local-first AI voice studio** — a free and open-source alternative to **ElevenLabs** and **WisprFlow** in one app. Clone voices from a few seconds of audio, generate speech in 23 languages across 7 TTS engines, dictate into any text field with a global hotkey, and give any MCP-aware AI agent a voice of your choosing.

The two cloud incumbents sit on opposite halves of the voice I/O loop — ElevenLabs on output, WisprFlow on input. Vox does both, bridges them with a bundled local LLM for refinement and per-profile personas, and runs the whole thing on your machine.
...
- **Agent voice output** — one tool call (`vox.speak`) and any MCP-aware agent (Claude Code, Cursor, Cline) speaks to you in a voice you've cloned
...
| Platform              | Download                                               |
| --------------------- | ------------------------------------------------------ |
| macOS (Apple Silicon) | [Download DMG](https://vox.sh/download/mac-arm)   |
| macOS (Intel)         | [Download DMG](https://vox.sh/download/mac-intel) |
| Windows               | [Download MSI](https://vox.sh/download/windows)   |
| Docker                | `docker compose up`                                    |

> **[View all binaries →](https://github.com/Clawbotsai/vox/releases/latest)**

> **Linux** — Pre-built binaries are not yet available. See [vox.sh/linux-install](https://vox.sh/linux-install) for build-from-source instructions.
...
The other half of the voice I/O loop. Hold a hotkey anywhere on your system, speak, release — on macOS the transcript pastes straight into the focused text field. Or hit the mic on any Vox text input and dictate directly into the app.
...
- **In-app mic button** on every Vox text field — generation form, profile descriptions, story titles, anywhere you'd type
...
Vox runs OpenAI Whisper for transcription — the same model that backs dictation, the Captures tab, and the `/transcribe` API. Running on MLX (Apple Silicon) or PyTorch (CUDA / ROCm / DirectML / CPU) depending on your platform.
...
- **Local capture storage** — original audio and transcript stay in your Vox data directory, with a folder shortcut in Settings
...
- **HTTP + stdio transports** — install as a URL in Claude Code / Cursor / Windsurf / VS Code MCP, or point stdio-only clients at the bundled `vox-mcp` binary
...
Agents can reach the same rewrite path over MCP by passing `personality: true` to `vox.speak`, turning the tool into a text-in → personality-LLM → TTS pipeline. The same LLM backs dictation's refinement step — one LLM in the app, one model cache, one GPU-memory footprint.
...
- Custom models directory via `VOX_MODELS_DIR`
...
Vox exposes a REST API for integrating voice I/O into your own apps and agents.
...
Vox ships a built-in **Model Context Protocol** server so any MCP-aware agent (Claude Code, Cursor, Windsurf, Cline, VS Code MCP extensions) can speak, transcribe, and browse captures and profiles.
...
claude mcp add vox \
  ...
  --header "X-Vox-Client-Id: claude-code"
```
...
```json
{
  "mcpServers": {
    "vox": {
      "url": "http://127.0.0.1:17493/mcp",
      "headers": { "X-Vox-Client-Id": "cursor" }
    }
  }
}
```

**Stdio fallback** for clients that don't speak HTTP MCP — point at the bundled `vox-mcp` binary inside the app:
...
```json
{
  "mcpServers": {
    "vox": {
      ...
      "env": { "VOX_CLIENT_ID": "claude-desktop" }
    }
  }
}
```

Four tools ship: `vox.speak`, `vox.transcribe`, `vox.list_captures`, `vox.list_profiles`. Per-client voice bindings are managed in **Vox → Settings → MCP**. See the [full MCP guide](docs/content/docs/overview/mcp-server.mdx) for tool signatures, resolution precedence, the speaking-pill contract, and security notes.
...
| **Mobile companion**               | Control Vox from your phone                                         |
...
```bash
git clone https://github.com/Clawbotsai/vox.git
cd vox
...
The repo ships a pre-wired `.mcp.json` at the root — running Claude Code inside this checkout picks up the Vox MCP tools automatically once the dev app is running.
