...
EXPORTS_DIR="tauri/assets/vox_exports"
ICON_BUNDLE="tauri/assets/vox.icon"
ASSETS_DIR="$ICON_BUNDLE/Assets"
ICONS_DIR="tauri/src-tauri/icons"
LANDING_LOGO="landing/public/vox-logo.png"
LANDING_PUBLIC="landing/public"
SOURCE_ICON="$EXPORTS_DIR/vox-iOS-Dark-1024x1024@1x.png"

echo "🎨 Updating all Vox icons from exports..."
echo ""
...
echo "Compiling vox.icon with actool..."
...
if [ -f "tauri/src-tauri/gen/vox.icns" ]; then
  echo "  ✓ vox.icns generated"
else
  echo "  ⚠ Warning: vox.icns not generated (will use fallback)"
fi
...
if [ -f "tauri/src-tauri/gen/vox.icns" ]; then
  cp tauri/src-tauri/gen/vox.icns "$ICONS_DIR/icon.icns"
  echo "  ✓ Copied Liquid Glass compiled icon.icns"
else
  echo "  ⚠ Liquid Glass icon not found, generating fallback icon.icns..."
mkdir -p /tmp/vox-iconset.iconset
sips -s format png -z 16 16 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_16x16.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_16x16@2x.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_32x32.png 2>/dev/null
sips -s format png -z 64 64 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_32x32@2x.png 2>/dev/null
sips -s format png -z 128 128 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_128x128.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_128x128@2x.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_256x256.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_256x256@2x.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_512x512.png 2>/dev/null
  sips -s format png -z 1024 1024 "$SOURCE_ICON" --out /tmp/vox-iconset.iconset/icon_512x512@2x.png 2>/dev/null
  iconutil -c icns /tmp/vox-iconset.iconset -o "$ICONS_DIR/icon.icns"
  rm -rf /tmp/vox-iconset.iconset
