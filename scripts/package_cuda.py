...
Takes the PyInstaller --onedir output directory and splits it into:
  1. vox-server-cuda.tar.gz  — server core (exe + non-NVIDIA deps)
  2. cuda-libs-cu128.tar.gz       — NVIDIA runtime libraries only
  3. cuda-libs.json                — version manifest for the CUDA libs

Usage:
    python scripts/package_cuda.py backend/dist/vox-server-cuda/
    python scripts/package_cuda.py backend/dist/vox-server-cuda/ --output release-assets/
    python scripts/package_cuda.py backend/dist/vox-server-cuda/ --cuda-libs-version cu128-v1
"""
...
def package(
    ...
    server_archive = output_dir / "vox-server-cuda.tar.gz"
    ...
    (output_dir / "vox-server-cuda.tar.gz.sha256").write_text(
        f"{server_sha}  vox-server-cuda.tar.gz\n"
    )
...
def main():
    ...
    parser.add_argument(
        "input",
        type=Path,
        help="Path to PyInstaller --onedir output directory (e.g. backend/dist/vox-server-cuda/)",
    )
    parser.add_argument(
        "--output",
        type=Path,
