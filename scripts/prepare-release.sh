...
if [ ! -f ~/.tauri/vox.key ]; then
  echo "❌ Private key not found at ~/.tauri/vox.key"
  echo "Run: cd tauri && bun tauri signer generate -w ~/.tauri/vox.key"
  exit 1
fi

if [ ! -f ~/.tauri/vox.key.pub ]; then
  echo "❌ Public key not found at ~/.tauri/vox.key.pub"
  exit 1
fi
...
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/vox.key)"
