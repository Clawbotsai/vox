...
Takes the PyInstaller --onedir output directory and splits it into:
  1. vox-server-rocm.tar.gz       — server core (exe + non-AMD deps)
...
Usage:
    python scripts/package_rocm.py backend/dist/vox-server-rocm/
    python scripts/package_rocm.py backend/dist/vox-server-rocm/ --output release-assets/
    python scripts/package_rocm.py backend/dist/vox-server-rocm/ --rocm-libs-version rocm7.2-v1
"""
...
def package(
    ...
    server_archive = output_dir / "vox-server-rocm.tar.gz"
    ...
    (output_dir / "vox-server-rocm.tar.gz.sha256").write_text(
        f"{server_sha}  vox-server-rocm.tar.gz\n"
    )
...
def main():
    ...
    parser.add_argument(
        "input",
        type=Path,
        help="Path to PyInstaller --onedir output directory (e.g. backend/dist/vox-server-rocm/)",
    )
    parser.add_argument(
        "--output",
        type=Path,
