---
title: "Why Vox has a token"
author: Jamie Pine
date: 2026-06-27
tags: [Token, Transparency]
excerpt: "Vox grew to over a million downloads with zero marketing — but donations never made full-time work sustainable. Here's the honest reasoning behind $VOX, and the commitments that come with it."
---

Vox started as a one-day experiment. The Qwen3-TTS model dropped, I wanted to try it, so I built a small CLI to load voice profiles and generate speech. As a designer I already had the interface in my head — profiles as cards, a floating generation box, a player spanning the bottom of the app, a gold accent and a microphone logo to make it feel like a real studio. First working version in a day. Open sourced in three.
...
The token changed that overnight. It's the closest thing I've had to a salary in a long time, and it let me go full-time on Vox immediately. That's not a hypothetical — it's already happening, and you'll see it in the commits, the releases, and the posts on [@VoxAI](https://x.com/VoxAI).

## What the token is — and what it isn't

**$VOX is entirely optional.** Vox is, and always will be, free, open source, and local-first. Every feature works without ever touching the token. It exists for supporters who want to back the project and have some fun — nothing here is gated behind it, and nothing here is financial advice.

It's also the **only** token I will ever make. My other projects, including Spacedrive, will never have an official token. I thought carefully about this: the community was clearly most interested in Vox because of its existing traction, and if I'm going to have a token at all, I'd rather deploy it myself so I can lock liquidity and be accountable for its trajectory — rather than have an anonymous community coin I can't control. As of now I no longer claim fees on any other community tokens, either.
...
- I've done **buyback and burns** — buying $VOX back from the market and burning it to a dead address, permanently. I've done this more than once, and I'll keep doing it.
...
- **More engines and broader hardware support.** More TTS engines, better GPU and OS coverage, so Vox runs great on whatever you've got.
