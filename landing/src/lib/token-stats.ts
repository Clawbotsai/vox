// Live on-chain stats for $VOX, fetched server-side via Helius.
//
// Design goals:
...
async function rpcCall<T>(rpc: string, method: string, params: unknown): Promise<T> {
  const res = await fetch(rpc, {
    ...
    body: JSON.stringify({ jsonrpc: '2.0', id: 'vox', method, params }),
  });
...
async function getCreatorRewardsSol(): Promise<number | null> {
  const res = await fetch(
    ...
    { next: { revalidate: REVALIDATE_S }, headers: { 'User-Agent': 'vox.sh' } },
  );
