...
- **Auto-paste into the focused app.** Once transcription finishes, Vox synthesizes a paste into whatever text field had focus when you started the chord — not wherever focus drifted while you were talking. Works across Dvorak / AZERTY layouts. Your clipboard is saved before and restored after.
...
**API surface.** `POST /generate`, `POST /speak`, and the MCP `vox.speak` tool accept `personality: bool`. `POST /profiles/{id}/compose` powers the shuffle button. MCP client bindings carry a `default_personality: bool` that applies when `personality` isn't passed explicitly.

### Agents — any MCP-aware agent gets a voice

Vox ships a built-in **Model Context Protocol** server at `http://127.0.0.1:17493/mcp` so Claude Code, Cursor, Windsurf, Cline, VS Code MCP extensions — any MCP-aware agent — can call into your local Vox install. Four tools ship with dotted names:

- **`vox.speak`** — speak text in any voice profile, with optional `personality: true` to run through the profile's personality LLM first
- **`vox.transcribe`** — Whisper transcription of a base64 blob or an absolute local path. Path mode is restricted to loopback callers so a Vox bound on `0.0.0.0` doesn't double as an unauthenticated arbitrary-local-file read primitive.
- **`vox.list_captures`** — recent captures with their transcripts
- **`vox.list_profiles`** — available voice profiles (cloned + preset)

- **Streamable HTTP as primary transport.** Cursor / Windsurf / VS Code / Claude Code all support it out of the box — drop a `mcpServers` block with the URL and an `X-Vox-Client-Id` header.
- **Stdio shim for clients that don't speak HTTP MCP.** A `vox-mcp` binary ships inside the app bundle as a Tauri sidecar. The Settings page renders the install snippet with the right absolute path pre-filled.
- **Per-client voice binding.** Pin Claude Code to Morgan, Cursor to Scarlett, Cline to its own voice — the `X-Vox-Client-Id` header resolves to a bound voice whenever `speak` is called without an explicit `profile`. Managed in **Settings → MCP**.
...
```
claude mcp add vox --transport http --url http://127.0.0.1:17493/mcp --header "X-Vox-Client-Id: claude-code"
```
...
A patch focused on two user-impacting reliability fixes: macOS DMG notarization (unblocks `brew install vox` on macOS 15 Sequoia and fixes spurious "app isn't signed" Gatekeeper dialogs on older Intel Macs) and Kokoro Japanese voice initialization on fresh installs.

### macOS

- **DMGs are now notarized and stapled** ([#523](https://github.com/Clawbotsai/vox/pull/523)). Tauri's bundler notarizes the `.app` inside the DMG but ships the DMG wrapper itself unnotarized. Gatekeeper rejects that on macOS 15 Sequoia (confirmed by Homebrew Cask CI failing on both arm and intel Sequoia runners) and causes the "the app is not signed" dialog on older Intel Macs when Apple's notarization servers are slow or unreachable ([#509](https://github.com/Clawbotsai/vox/issues/509)). The release workflow now submits each DMG to `notarytool`, staples the ticket, verifies with `spctl`, and overwrites the draft-release asset `tauri-action` uploaded. Adds ~5-10 min per macOS job.

### Backend

- **Kokoro Japanese voices no longer crash on fresh installs** ([#521](https://github.com/Clawbotsai/vox/pull/521), fixes [#514](https://github.com/Clawbotsai/vox/issues/514)). `misaki[ja]` pulls in `fugashi`, which needs a MeCab dictionary on disk. The `unidic` package that was being installed ships no data and expects a ~526MB runtime download that `just setup` doesn't run (and which wouldn't survive PyInstaller anyway). Swapped to `unidic-lite`, which bundles a MeCab-compatible dict inside the wheel (~50MB). Collected in `build_binary.py` so frozen builds pick up `unidic_lite/dicdir/`.
...
### Internationalization ([#508](https://github.com/Clawbotsai/vox/pull/508))
...
### Reliability
- **`HF_HUB_OFFLINE` now guards every inference path** ([#503](https://github.com/Clawbotsai/vox/pull/503)) — some engines were still attempting a HuggingFace metadata roundtrip on first load when offline mode was enabled, causing hangs on airgapped or flaky networks.
- **Chatterbox reference samples are preprocessed instead of rejected** ([#502](https://github.com/Clawbotsai/vox/pull/502)) — samples outside the expected sample rate or channel layout are resampled to match, rather than failing with an opaque error.
- **MLX Qwen 0.6B repo path fixed** ([#501](https://github.com/Clawbotsai/vox/pull/501)) — now points at the published `mlx-community` repo so the model actually downloads on Apple Silicon.
- **macOS system audio survives backgrounding** ([#486](https://github.com/Clawbotsai/vox/pull/486), closes [#41](https://github.com/Clawbotsai/vox/issues/41)) — WKWebView was tearing down the audio session when the app lost focus, silently killing system-audio capture.
- **MLX backend `miniaudio` dependency pinned** ([#506](https://github.com/Clawbotsai/vox/pull/506)) — `mlx_audio.stt` needs it at runtime and nothing else transitively pulled it in, so `--no-deps` installs were breaking on first use.

### Landing / Docs
- **New `/download` page** ([#487](https://github.com/Clawbotsai/vox/pull/487)) — no more dumping first-time visitors onto the GitHub releases list. The API example snippet on the landing page also got an accuracy pass.
- **Download redirects work behind reverse proxies** ([#498](https://github.com/Clawbotsai/vox/pull/498)) — uses the public origin instead of `localhost` when resolving platform-specific installer URLs.
- **MDX docs audited against the multi-engine backend** ([#484](https://github.com/Clawbotsai/vox/pull/484)) — stale single-engine assumptions removed.
- **Three more tutorials + mobile navbar / hero CTA fixes** ([#483](https://github.com/Clawbotsai/vox/pull/483)).

### Linux
- **Still not shipping.** The re-enable attempt ([#488](https://github.com/Clawbotsai/vox/pull/488)) landed on `main` but CI still hangs in the `tauri-action` bundler step on `ubuntu-22.04` — no output for 25+ minutes after `rpm` bundling, even with `createUpdaterArtifacts: false` and `--bundles deb,rpm`. The matrix entry is disabled again for 0.4.2; the ubuntu-specific setup steps stay in the workflow so re-enabling is a one-line change once we identify the hang. Next release will take another pass.

### New Contributors
- [@shekharyv](https://github.com/shekharyv) — download redirects behind reverse proxies ([#498](https://github.com/Clawbotsai/vox/pull/498))
...
### Frozen-Binary Reliability ([#438](https://github.com/Clawbotsai/vox/pull/438))
...
### Generation
- **Cancel queued or running generations** ([#444](https://github.com/Clawbotsai/vox/pull/444)) — new `/generate/{id}/cancel` endpoint and a Stop button on the history row while generating. The serial queue now tracks per-ID state (queued / running / cancelled) so queued jobs are skipped before the worker picks them up and running jobs are `.cancel()`-ed mid-flight; `run_generation` catches `CancelledError` and marks the row `failed` with a "cancelled" error.
- **Legacy `data/` path prefix resolution** ([#440](https://github.com/Clawbotsai/vox/pull/440)) — generations stored with the old `data/` prefix under pre-0.4 installs now resolve correctly after the storage root moved, fixing 404s for historical audio.

### Model Migration
- Migration dialog no longer hangs when the cache is empty ([#439](https://github.com/Clawbotsai/vox/pull/439)) — the backend now emits a completion SSE event even when zero models are moved.
- Storage-change flow surfaces a toast when there's nothing to migrate ([#433](https://github.com/Clawbotsai/vox/pull/433)) instead of proceeding with a no-op move and restarting the server.
- Deleting all generations from a voice profile now deletes the associated version files and DB rows too ([#447](https://github.com/Clawbotsai/vox/pull/447)) — previously orphaned versions accumulated in storage.

### Platform
- **Linux system audio capture** ([#457](https://github.com/Clawbotsai/vox/pull/457)) — `cpal`'s ALSA backend doesn't expose PulseAudio/PipeWire monitor sources by name, so the previous device-name search never matched and silently fell back to the microphone. Detection now uses `pactl get-default-sink` + `pactl list short sources` and routes via `PULSE_SOURCE`, with the name-based search retained as a fallback when `pactl` is absent.

### Frontend CI
- First PR-time quality gate ([#418](https://github.com/Clawbotsai/vox/pull/418)) — new `.github/workflows/ci.yml` runs `bun run typecheck` + `bun run build:web` on every PR. Fixed pre-existing type issues that were being suppressed with `@ts-expect-error`, cleaned up a dep-array typo (`[platform.metadata.isTauricheckOnMountcheckForUpdates]`) in `useAutoUpdater`, and removed 100+ lines of dead `ModelItem` code from `ModelManagement.tsx`.
- Follow-up: widened `apiClient.migrateModels()` return type to include `moved` and `errors` so the storage-change handler typechecks against the real backend response ([#470](https://github.com/Clawbotsai/vox/pull/470)).

### Docs
- Clarified in the Quick Start + README that paralinguistic tags (`[laugh]`, `[sigh]`) only work with Chatterbox Turbo; other engines read them as literal text ([#450](https://github.com/Clawbotsai/vox/pull/450)).
...
#### HumeAI TADA — Expressive English & Multilingual ([#296](https://github.com/Clawbotsai/vox/pull/296))
...
#### Kokoro 82M — Fast Lightweight TTS ([#325](https://github.com/Clawbotsai/vox/pull/325))
...
#### Qwen CustomVoice ([#328](https://github.com/Clawbotsai/vox/pull/328))
...
#### Intel Arc (XPU) Support ([#320](https://github.com/Clawbotsai/vox/pull/320))
...
#### Blackwell / RTX 50-series Support ([#316](https://github.com/Clawbotsai/vox/pull/316), [#401](https://github.com/Clawbotsai/vox/pull/401))
...
#### GPU Compatibility Diagnostics ([#367](https://github.com/Clawbotsai/vox/pull/367), adapted)
...
#### Split CUDA Backend ([#298](https://github.com/Clawbotsai/vox/pull/298))
...
- Upgrading Vox no longer redownloads the libs archive when only the server binary changed
- Added `asyncio.Lock` around `download_cuda_binary()` so auto-update and manual download can't race on the same temp file ([#428](https://github.com/Clawbotsai/vox/pull/428))
...
- **numpy 2.x `torch.from_numpy` crash** ([#361](https://github.com/Clawbotsai/vox/pull/361)) — torch compiled against numpy 1.x ABI fails silently when paired with numpy 2.x, causing `RuntimeError: Numpy is not available` / `Unable to create tensor` on every TTS request in bundled macOS Intel / Rosetta builds. Pinned `numpy<2.0` in requirements and added a PyInstaller runtime hook with a `ctypes.memmove` fallback as belt-and-suspenders. Hardened afterward to raise on unknown dtypes instead of silently reinterpreting bytes as float32.

#### Platform Reliability
- **Windows background server** ([#402](https://github.com/Clawbotsai/vox/pull/402)) — "keep server running after close" now actually keeps the server running. The HTTP `/watchdog/disable` request could lose the race against process exit on Windows; added a `.keep-running` sentinel file as a synchronous fallback, with stale-sentinel cleanup on startup to avoid orphan server processes
- **macOS 11 launch crash** ([#424](https://github.com/Clawbotsai/vox/pull/424)) — weak-linked ScreenCaptureKit so the app can launch on macOS < 12.3 instead of crashing at dyld resolution. Gated system audio capture behind a real `sw_vers` version check so unsupported systems cleanly advertise "not available" rather than crashing at runtime
- **macOS Intel (x86_64) setup** ([#416](https://github.com/Clawbotsai/vox/pull/416)) — relaxed `torch>=2.7.0` → `torch>=2.2.0`. PyTorch dropped pre-built x86_64 wheels after 2.2.2, so Intel Mac devs could no longer `pip install`. Now resolves to the latest compatible torch per platform
- **Offline model loading** ([#318](https://github.com/Clawbotsai/vox/pull/318)) — Qwen TTS and Whisper force offline mode when loading cached models, so startup works without network access
- **GUI startup with external server** ([#319](https://github.com/Clawbotsai/vox/pull/319)) — fixed GUI launch when pointed at a remote/external server, and added data refresh on server switch; hardened health validation and error handling
- **Qwen3-TTS cache split on Windows** (adapted from [#218](https://github.com/Clawbotsai/vox/pull/218)) — route `Qwen3TTSModel.from_pretrained` through `hf_constants.HF_HUB_CACHE` so the speech tokenizer and `preprocessor_config.json` resolve from a single cache root
- **Qwen3-TTS bundling** ([#305](https://github.com/Clawbotsai/vox/pull/305)) — bundle `qwen_tts` source files in the PyInstaller build to fix `inspect.getsource` errors in frozen builds
- **Backend import paths** ([#345](https://github.com/Clawbotsai/vox/pull/345)) — moved lazy imports to top-level with absolute paths to resolve the "Failed to Save" preset error caused by `ModuleNotFoundError` in production builds
- **Effects service import** ([#384](https://github.com/Clawbotsai/vox/pull/384)) — fixed `ModuleNotFoundError` on preset create/update by switching to relative imports (#349)

#### Audio & Playback
- **cpal stream silent playback** ([#405](https://github.com/Clawbotsai/vox/pull/405)) — `cpal::Stream` was dropped on function return immediately after `play()`, causing every playback to fall silent. Now holds the stream until either the buffer drains or the stop flag fires (#404)

#### Stories & History
- **Clip-splitting race** ([#403](https://github.com/Clawbotsai/vox/pull/403)) — rapid double-clicks on split could race through `split_story_item` with inconsistent state. Added `with_for_update()` row locking on the backend and an `isPending` guard on the frontend (#366)
- **History `status` staleness** ([#394](https://github.com/Clawbotsai/vox/pull/394)) — `GET /history/{id}` was hardcoding `status="completed"` regardless of the DB row, breaking any client polling for job completion. Now returns `status`, `error`, `engine`, `model_size`, and `is_favorited` from the actual row
- **"Clear failed" bulk button** ([#412](https://github.com/Clawbotsai/vox/pull/412)) — new `DELETE /history/failed` endpoint and a header strip showing `"N failed generations"` with a Clear button, complementing the per-row trash icon added in #321 (#410)
- **Delete failed generations** ([#321](https://github.com/Clawbotsai/vox/pull/321)) — added a trash icon next to the retry button so failed entries can be cleaned up without having to retry first

#### Security & Safety
- **Voice prompt cache hardening** ([#429](https://github.com/Clawbotsai/vox/pull/429)) — `torch.load(weights_only=True)` on cached voice prompts per PyTorch 2.6 recommendation; replaced string-based SPA path guard with `Path.is_relative_to()` for more robust path-traversal protection

#### Infrastructure & Docker
- **Docker web build** ([#344](https://github.com/Clawbotsai/vox/pull/344)) — include `CHANGELOG.md` in the Docker web build so the in-app changelog page works in Docker deployments
- **Docker numba cache** ([#425](https://github.com/Clawbotsai/vox/pull/425)) — set `NUMBA_CACHE_DIR` in docker-compose so numba can write its JIT cache in container runtime (#308)
- **Relative media paths** ([#332](https://github.com/Clawbotsai/vox/pull/332)) — media paths now stored relative to the configured data dir rather than resolved against CWD, so the data directory is portable between installs
...
- Fixed broken links in the Get Started section ([#332](https://github.com/Clawbotsai/vox/pull/332))

### New Contributors

Huge thank you to everyone who contributed their first PR to Vox in this release:
...
### Settings Overhaul ([#294](https://github.com/Clawbotsai/vox/pull/294))
...
### Audio Player Fix ([#293](https://github.com/Clawbotsai/vox/pull/293))
...
### Backend Refactor ([#285](https://github.com/Clawbotsai/vox/pull/285))
...
### Documentation Rewrite ([#288](https://github.com/Clawbotsai/vox/pull/288))
...
### UI & Frontend
- Added model loading status indicator and effects preset dropdown ([3187344](https://github.com/Clawbotsai/vox/commit/3187344))
...
### Bug Fixes ([#295](https://github.com/Clawbotsai/vox/pull/295))
- Fixed sample uploads crashing the server — audio decoding now runs in a thread pool instead of blocking the async event loop ([#278](https://github.com/Clawbotsai/vox/issues/278))
- Fixed generation list not updating when a generation completes — switched to `refetchQueries` for reliable cache busting, added SSE error fallback, and page reset on completion ([#231](https://github.com/Clawbotsai/vox/issues/231))
- Fixed error toasts showing `[object Object]` instead of the actual error message ([#290](https://github.com/Clawbotsai/vox/issues/290))
- Added Whisper model selection (`base`, `small`, `medium`, `large`, `turbo`) and expanded language support to the `/transcribe` endpoint ([#233](https://github.com/Clawbotsai/vox/issues/233))
- Upgraded CUDA backend build from cu121 to cu126 for RTX 50-series (Blackwell) GPU support ([#289](https://github.com/Clawbotsai/vox/issues/289))
- Handled client disconnects in SSE and streaming endpoints to suppress `[Errno 32] Broken Pipe` errors ([#248](https://github.com/Clawbotsai/vox/issues/248))
- Fixed Docker build failure from pip hash mismatch on Qwen3-TTS dependencies ([#286](https://github.com/Clawbotsai/vox/issues/286))
...
### Platform Fixes
- Replaced `netstat` with `TcpStream` + PowerShell for Windows port detection ([#277](https://github.com/Clawbotsai/vox/pull/277))
...
Vox v0.1.x was a single-engine voice cloning app built around Qwen3-TTS. v0.2.0 is a ground-up rethink: four TTS engines, 23 languages, paralinguistic emotion controls, a post-processing effects pipeline, unlimited generation length, an async generation queue, and support for every major GPU vendor. Plus Docker.

### New TTS Engines

#### Multi-Engine Architecture

Vox now runs **four independent TTS engines** behind a thread-safe per-engine backend registry. Switch engines per-generation from a single dropdown — no restart required.
...
#### Chatterbox Multilingual — 23 Languages ([#257](https://github.com/Clawbotsai/vox/pull/257))
...
#### LuxTTS — Lightweight English TTS ([#254](https://github.com/Clawbotsai/vox/pull/254))

A fast, CPU-friendly English engine. ~300 MB download, 48 kHz output, runs at 150x realtime on CPU.

#### Chatterbox Turbo — Expressive English ([#258](https://github.com/Clawbotsai/vox/pull/258))

A fast 350M-parameter English model with inline paralinguistic tags.

#### Paralinguistic Tags Autocomplete ([#265](https://github.com/Clawbotsai/vox/pull/265))
...
#### Unlimited Generation Length — Auto-Chunking ([#266](https://github.com/Clawbotsai/vox/pull/266))
...
#### Asynchronous Generation Queue ([#269](https://github.com/Clawbotsai/vox/pull/269))
...
### Post-Processing Effects ([#271](https://github.com/Clawbotsai/vox/pull/271))
...
- **Windows Support** ([#272](https://github.com/Clawbotsai/vox/pull/272)) — Full Windows support with CUDA GPU detection
- **Linux** ([#262](https://github.com/Clawbotsai/vox/pull/262)) — AMD ROCm, NVIDIA GBM fix, WebKitGTK mic access (build from source)
- **NVIDIA CUDA Backend Swap** ([#252](https://github.com/Clawbotsai/vox/pull/252)) — Download and swap in CUDA backend from within the app
- **Intel Arc (XPU) and DirectML** — PyTorch backend supports Intel Arc and DirectML
- **Docker + Web Deployment** ([#161](https://github.com/Clawbotsai/vox/pull/161)) — 3-stage build, non-root runtime, health checks
- **Whisper Turbo** — Added `openai/whisper-large-v3-turbo` as a transcription model option

### Model Management ([#268](https://github.com/Clawbotsai/vox/pull/268))

Per-model unload, custom models directory, model folder migration, download cancel/clear UI ([#238](https://github.com/Clawbotsai/vox/pull/238)), restructured settings UI.

### Security & Reliability

- CORS hardening ([#88](https://github.com/Clawbotsai/vox/pull/88))
- Network access toggle ([#133](https://github.com/Clawbotsai/vox/pull/133))
- Offline crash fix ([#152](https://github.com/Clawbotsai/vox/pull/152))
- Atomic audio saves ([#263](https://github.com/Clawbotsai/vox/pull/263))
- Filesystem health endpoint
- Chatterbox float64 dtype fix ([#264](https://github.com/Clawbotsai/vox/pull/264))

### Accessibility ([#243](https://github.com/Clawbotsai/vox/pull/243))

Screen reader support, keyboard navigation, state-aware `aria-label` attributes on all interactive controls.

### UI Polish

- Redesigned landing page ([#274](https://github.com/Clawbotsai/vox/pull/274))
- Voices tab overhaul with inline inspector
- Responsive layout improvements
- Duplicate profile name validation ([#175](https://github.com/Clawbotsai/vox/pull/175))
...
- [#95](https://github.com/Clawbotsai/vox/pull/95) Fix: selecting 0.6B model still downloads and uses 1.7B
- [#93](https://github.com/Clawbotsai/vox/pull/93) fix(mlx): bundle native libs and broaden error handling for Apple Silicon
- [#79](https://github.com/Clawbotsai/vox/pull/79) fix: handle non-ASCII filenames in Content-Disposition headers
- [#78](https://github.com/Clawbotsai/vox/pull/78) fix: guard getUserMedia call against undefined mediaDevices in non-secure contexts
- [#77](https://github.com/Clawbotsai/vox/pull/77) fix: await for confirmation before deleting voices and channels
- [#128](https://github.com/Clawbotsai/vox/pull/128) fix: resolve multiple issues (#96, #119, #111, #108, #121, #125, #127)
- [#40](https://github.com/Clawbotsai/vox/pull/40) Fix: audio export path resolution

### Build and packaging

- [#122](https://github.com/Clawbotsai/vox/pull/122) fix(web): add @tailwindcss/vite plugin to web config
- [#126](https://github.com/Clawbotsai/vox/pull/126) Create requirements.txt

### UX and docs

- [#44](https://github.com/Clawbotsai/vox/pull/44) Enhances floating generate box UX
- [#57](https://github.com/Clawbotsai/vox/pull/57) chore: updates repo URL in README
- [#146](https://github.com/Clawbotsai/vox/pull/146) Add Spacebot banner to landing page
- [#1](https://github.com/Clawbotsai/vox/pull/1) Improvements
...
The first public release of Vox — an open-source voice synthesis studio powered by Qwen3-TTS.
...
[0.5.0]: https://github.com/Clawbotsai/vox/compare/v0.4.5...v0.5.0
[0.4.5]: https://github.com/Clawbotsai/vox/compare/v0.4.4...v0.4.5
[0.4.4]: https://github.com/Clawbotsai/vox/compare/v0.4.3...v0.4.4
[0.4.3]: https://github.com/Clawbotsai/vox/compare/v0.4.2...v0.4.3
[0.4.2]: https://github.com/Clawbotsai/vox/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/Clawbotsai/vox/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/Clawbotsai/vox/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/Clawbotsai/vox/compare/v0.2.3...v0.3.0
[0.2.3]: https://github.com/Clawbotsai/vox/compare/v0.2.2...v0.2.3
[0.2.2]: https://github.com/Clawbotsai/vox/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/Clawbotsai/vox/compare/v0.1.13...v0.2.1
[0.1.13]: https://github.com/Clawbotsai/vox/compare/v0.1.12...v0.1.13
[0.1.12]: https://github.com/Clawbotsai/vox/compare/v0.1.11...v0.1.12
[0.1.11]: https://github.com/Clawbotsai/vox/compare/v0.1.10...v0.1.11
[0.1.10]: https://github.com/Clawbotsai/vox/compare/v0.1.9...v0.1.10
[0.1.9]: https://github.com/Clawbotsai/vox/compare/v0.1.8...v0.1.9
[0.1.8]: https://github.com/Clawbotsai/vox/compare/v0.1.7...v0.1.8
[0.1.7]: https://github.com/Clawbotsai/vox/compare/v0.1.6...v0.1.7
[0.1.6]: https://github.com/Clawbotsai/vox/compare/v0.1.5...v0.1.6
[0.1.5]: https://github.com/Clawbotsai/vox/compare/v0.1.4...v0.1.5
[0.1.4]: https://github.com/Clawbotsai/vox/compare/v0.1.3...v0.1.4
[0.1.3]: https://github.com/Clawbotsai/vox/compare/v0.1.2...v0.1.3
[0.1.2]: https://github.com/Clawbotsai/vox/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/Clawbotsai/vox/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/Clawbotsai/vox/releases/tag/v0.1.0
