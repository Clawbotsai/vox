...
- `paste_final_text` command — activate → 120 ms settle → save clip →
  write text → ⌘V → 400 ms → restore. Skips when focus was in Vox
  itself.
...
direct-injection path for focus-was-inside-Vox (step 6 — dictating
...
- **Phase 5 — Agent voice output + persona loop.** No `/speak` endpoint, no
  `vox.speak` MCP tool, no per-agent voice binding, no persona metadata
  on profiles.
...
Vox ships the output half of a voice I/O loop: clone a voice, generate
...
Positioning is the key move. **Vox becomes the local voice I/O layer for
...
cloned. "Anything voice is Vox."

### Positioning shift

Before this plan, Vox was **"the open-source AI voice cloning studio."**
Cloning was the headline capability.

After this plan, Vox is **"the open-source AI voice studio."** Cloning is
...
Vox ends up covering the territory of two separately-funded, separately
...
Both are cloud-only. Vox becomes the only local alternative to either,
...
- Agent harnesses already pipe Vox TTS into their stacks. Giving those
  users STT from the same app closes the loop and makes Vox the default
  voice I/O layer for the agentic dev-tool crowd.
- **Typing a 2,000-character TTS script is user-hostile.** The most immediate
  internal win is dictating directly into Vox's own generation form —
...
- A separate tray-only dictation app. We extend Vox, not fork it.
...
Vox-native language (see open questions).
...
|---|---|
| Focused text field inside Vox | Direct React state update via event. No clipboard involved. |
...
Vox's dictation story. The Generate tab deserves a parallel — first-time
...
The shape is a single new capability: any agent can call Vox to speak
...
```
MCP tool:  vox.speak({ text, profile?, style? })
...
This primitive is what makes "Vox as voice layer for every agent on your
...
1. **Tab name.** Leaning **Captures** — neutral, extensible across dictation,
   ...
   recordings). "Notes" is the wrong mental model — nobody opens Vox to
...
6. **Hotkey primitive naming.** Hold-vs-tap needs Vox-native phrasing in
...
A universal mic button on every Vox text input. Hold, speak, release —
text lands in the focused field via direct React state update. No OS APIs
involved; Vox owns the input.
...
onward: Superwhisper and WisprFlow cannot do this because they have no TTS. Vox can, with one day of frontend wiring.
...
Two features that together make "Vox as the voice layer for every agent
on your machine" a shipping reality:

1. **`speak()` primitive.** New `POST /speak` endpoint and `vox.speak`
   MCP tool. Any agent calls Vox to speak arbitrary text in a
