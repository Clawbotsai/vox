...
Docker support makes Vox easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v vox-data:/app/data \
  ghcr.io/Clawbotsai/vox:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v vox-data:/app/data \
  ghcr.io/Clawbotsai/vox:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v vox-data:/app/data \
  ghcr.io/Clawbotsai/vox:latest-rocm
```
...
services:
  vox:
    image: ghcr.io/Clawbotsai/vox:latest-cuda
    ports:
      - "8000:8000"
    volumes:
      - vox-data:/app/data
...
```bash
docker build -t vox .
docker run -p 8000:8000 -v $(pwd)/data:/app/data vox
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t vox:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v vox-data:/app/data \
  vox:rocm
```
...
```bash
docker run -v vox-data:/app/data \           # Profiles, generations, history
...
Configure Vox via environment variables:
...
doctl compute droplet create vox \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 vox:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/vox .
docker push gcr.io/your-project/vox

# Deploy to Cloud Run
gcloud run deploy vox \
  --image gcr.io/your-project/vox \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/Clawbotsai/vox:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data vox
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 vox
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu vox
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface vox
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface vox
```
...
Change host port:
```bash
docker run -p 8080:8000 vox  # Use port 8080 instead
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 vox

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true vox
```
...
4. Report issues: [GitHub Issues](https://github.com/Clawbotsai/vox/issues)
