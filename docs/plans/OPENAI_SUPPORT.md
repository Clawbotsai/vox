...
**Issue:** [#10 OpenAI API compatibility](https://github.com/Clawbotsai/vox/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from Vox, allowing any tool, library, or application that speaks the OpenAI Audio API to use Vox as a drop-in local replacement.
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Vox
- **LLM frameworks**: LangChain, AutoGen, etc. can use Vox for TTS
...
- `voice` parameter maps to Vox profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Vox profile.
