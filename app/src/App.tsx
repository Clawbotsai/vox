...
/**
 * Validate that a health response has the expected Vox-specific shape.
 * Prevents misidentifying an unrelated service on the same port.
 */
function isVoxHealthResponse(health: HealthResponse): boolean {
  return (
...
function MainApp() {
  ...
  useEffect(() => {
    if (!platform.metadata.isTauri) {
      ...
      if (currentServerUrl !== serverUrl && isLoopbackVoxServerUrl(currentServerUrl)) {
    ...
    if (!import.meta.env?.PROD) {
      ...
      window.__voxServerStartedByApp = false;
      return;
    }
    ...
    platform.lifecycle
      ...
      .then((serverUrl) => {
        ...
        window.__voxServerStartedByApp = true;
      })
      .catch((error) => {
        ...
        // until it responds with a valid Vox payload, then transition to
        // the main UI.
        console.log('Falling back to health-check polling...');
        const pollInterval = setInterval(async () => {
          try {
            const health = await apiClient.getHealth();
            if (!isVoxHealthResponse(health)) {
              console.log('Health response is not from a Vox server, keep polling...');
              return;
            }
            console.log('External Vox server detected via health check');
        ...
        setTimeout(() => {
          ...
          serverStartingRef.current = false;
          setStartupError(
            'Could not connect to a Vox server within 2 minutes. ' +
