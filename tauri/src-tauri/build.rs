...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        // vox.icon is in tauri/assets/vox.icon (one level up from src-tauri)
        let icon_source = format!("{}/../assets/vox.icon", project_root);
        ...
        if std::path::Path::new(&icon_source).exists() {
            ...
            // Generate vox.icns from the source PNG via sips + iconutil
            let icns_path = format!("{}/vox.icns", gen_dir);
            if !std::path::Path::new(&icns_path).exists() {
                let source_png = format!("{}/Assets/Vox.png", icon_source);
                if std::path::Path::new(&source_png).exists() {
                    let iconset_dir = format!("{}/vox.iconset", gen_dir);
    ...
    {
        let required = ["Assets.car", "vox.icns", "partial.plist"];
