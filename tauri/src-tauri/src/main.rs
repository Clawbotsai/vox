...
/// Find a vox-server process listening on a given port (Windows only).
///
/// Uses PowerShell `Get-NetTCPConnection` to look up the PID owning the port,
/// then verifies via `tasklist` that it's a vox process. The caller is
...
#[cfg(windows)]
fn find_vox_pid_on_port(port: u16) -> Option<u32> {
...
/// Check if a Vox server is responding on the given port.
///
/// Sends an HTTP GET to `/health` and returns `true` only if the response
/// is valid JSON matching the Vox `HealthResponse` schema — specifically
        ...
        .build()
    {
        Ok(client) => match client.get(&url).send() {
            Ok(resp) => {
                ...
                // Parse as JSON and validate Vox-specific fields
...
async fn start_server(
    ...
    // Check if a vox server is already running on our port (from previous session with keep_running=true,
            ...
            .output()
        {
            ...
            for line in output_str.lines().skip(1) {
                ...
                if parts.len() >= 2 {
                    let command = parts[0];
                    let pid_str = parts[1];
                    if command.contains("vox") {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            println!("Found existing vox-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                            // Store the PID so we can kill it on exit if needed
                            *state.server_pid.lock().unwrap() = Some(pid);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                    } else {
                        // Process name doesn't contain "vox" — could be an external
                        // Python/uvicorn/Docker server. Verify via HTTP health check.
                        println!("Port {} in use by '{}' (PID: {}), checking if it's a Vox server...", SERVER_PORT, command, pid_str);
                        if check_health(SERVER_PORT) {
                            println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                        println!("Health check failed — port is occupied by a non-Vox process");
                        return Err(format!(
                            "Port {} is already in use by another application ({}). \
                             Close it or change the Vox server port.",
                            SERVER_PORT, command
                        ));
                    }
                }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            // Port is in use — check if it's a vox process by name first
            if let Some(pid) = find_vox_pid_on_port(SERVER_PORT) {
                println!("Found existing vox-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                *state.server_pid.lock().unwrap() = Some(pid);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            ...
            println!("Port {} in use by unknown process, checking if it's a Vox server...", SERVER_PORT);
            if check_health(SERVER_PORT) {
                println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            return Err(format!(
                "Port {} is already in use by another application. \
                 Close the other application or change the Vox port.",
                SERVER_PORT
            ));
        }
    }

    // Kill any orphaned vox-server from previous session on legacy port 8000
                ...
                if parts.len() >= 2 {
                    ...
                    if command.contains("vox") {
                        if let Ok(pid) = pid_str.parse::<i32>() {
                            println!("Found orphaned vox-server on legacy port {} (PID: {}, CMD: {}), killing it...", LEGACY_PORT, pid, command);
                        ...
                        println!("Legacy port {} is in use by non-vox process: {} (PID: {}), not killing", LEGACY_PORT, command, pid_str);
                    }
                }
            }
        }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            if let Some(pid) = find_vox_pid_on_port(LEGACY_PORT) {
                println!("Found orphaned vox-server on legacy port {} (PID: {}), killing it...", LEGACY_PORT, pid);
    ...
    println!("=================================================================");
    println!("Starting vox-server sidecar");
    ...
    let sidecar_result = app.shell().sidecar("vox-server");
    ...
    let spawn_result = if backend_override.as_deref() != Some("cpu") {
        ...
        if let Some(ref rocm_path) = rocm_binary {
            ...
            if let Some(ref dir) = effective_models_dir { cmd = cmd.env("VOX_MODELS_DIR", dir); }
            match cmd.spawn() {
        ...
        if let Some(result) = gpu_spawn {
            ...
            if let Some(ref dir) = effective_models_dir { sidecar = sidecar.env("VOX_MODELS_DIR", dir); }
        ...
        if let Some(ref dir) = effective_models_dir {
            sidecar = sidecar.env("VOX_MODELS_DIR", dir);
        }
        sidecar.spawn()
    };
        ...
        match tokio::time::timeout(tokio::time::Duration::from_millis(100), rx.recv()).await {
            ...
            Ok(None) => {
                ...
                #[cfg(not(debug_assertions))]
                {
                    ...
                    eprintln!("Check Console.app logs for more details (search for 'vox')");
...
/// Identifier of the Vox app itself — used to short-circuit auto-paste
/// when the user fires a chord while focus was inside one of our own
/// windows. Paste into Vox-internal targets is step 6 territory and
...
#[cfg(target_os = "macos")]
const VOX_BUNDLE_ID: &str = "sh.vox.app";
#[cfg(target_os = "windows")]
const VOX_BUNDLE_ID: &str = "vox.exe";
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const VOX_BUNDLE_ID: &str = "vox";
...
///
/// On macOS this is the call that triggers the "Vox would like to receive
...
/// - `focus.bundle_id` is Vox itself — step 6 will inject directly
...
async fn paste_final_text(
    ...
    if focus.bundle_id.as_deref() == Some(VOX_BUNDLE_ID) {
        return Ok(false);
    }
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission required for auto-paste. Open System Settings → Privacy & Security → Accessibility and enable Vox."
                .into(),
        );
    }
    ...
    if safe_to_restore {
        ...
        eprintln!(
            "[vox] clipboard mutated during paste window — skipping restore to preserve newer content"
        );
    }
...
async fn debug_focus_roundtrip(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Vox."
                .into(),
        );
    }
...
async fn debug_paste_text(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Vox, then try again."
                .into(),
        );
    }
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                // steals focus to the Vox app when the user clicks
                ...
                // Agent-initiated speech (vox.speak over MCP or POST /speak)
