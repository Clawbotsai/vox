...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "vox-server-cuda" / f"vox-server-cuda{exe}",
        DIST_DIR / f"vox-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# Vox E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Vox E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to vox-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="vox-e2e-"))
