...
def fake_tar_gz():
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        ...
        info = tarfile.TarInfo(name="vox-server-rocm.exe")
        info.size = len(data)
...
async def test_download_rocm_binary_progress_reporting(mock_backends_dir, fake_tar_gz, fake_sha256):
    ...
    responses = {
        "https://github.com/Clawbotsai/vox/releases/download/v0.2.3/vox-server-rocm.tar.gz": FakeResponse(
            content=fake_tar_gz,
            headers={"content-length": str(len(fake_tar_gz))},
        ),
        "https://github.com/Clawbotsai/vox/releases/download/v0.2.3/vox-server-rocm.tar.gz.sha256": FakeResponse(
            content=f"{server_sha}  vox-server-rocm.tar.gz\n".encode(),
        ),
        f"https://github.com/Clawbotsai/vox/releases/download/v0.2.3/rocm-libs-{rocm.ROCM_LIBS_VERSION}.tar.gz": FakeResponse(
            content=fake_tar_gz,
            headers={"content-length": str(len(fake_tar_gz))},
        ),
        f"https://github.com/Clawbotsai/vox/releases/download/v0.2.3/rocm-libs-{rocm.ROCM_LIBS_VERSION}.tar.gz.sha256": FakeResponse(
    ...
    assert (rocm_dir / "vox-server-rocm.exe").exists()
...
async def test_is_rocm_active(mock_backends_dir, monkeypatch):
    monkeypatch.setenv("VOX_BACKEND_VARIANT", "rocm")
    assert rocm.is_rocm_active() is True

    monkeypatch.setenv("VOX_BACKEND_VARIANT", "cpu")
    assert rocm.is_rocm_active() is False

    monkeypatch.delenv("VOX_BACKEND_VARIANT", raising=False)
    assert rocm.is_rocm_active() is False
