...
|----------|------|------------|
| macOS    | `backend/dist/vox-server-cuda/vox-server-cuda` | onedir (CUDA, rarely on Mac) |
| macOS    | `backend/dist/vox-server`                          | onefile (CPU) |
| Windows  | `backend\dist\vox-server-cuda\vox-server-cuda.exe` | onedir (CUDA) |
| Windows  | `backend\dist\vox-server.exe`                      | onefile (CPU) |
...
- **Data dir**: `tempfile.mkdtemp(prefix="vox-e2e-")`. Deleted after the run unless `--keep-data-dir`. Profiles and generated WAVs land here.
...
```
# Vox E2E — darwin-arm64 — 2026-04-16 12:34
...
- Don't touch the user's HF cache by default — let the server use `HF_HUB_CACHE` / `VOX_MODELS_DIR`. Passing `--isolated-cache` would point both env vars at the tempdir for a true cold-start run (opt-in only; would re-download every time).
