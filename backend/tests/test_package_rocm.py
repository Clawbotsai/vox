...
class TestPackage:
    ...
    def test_split_and_manifest(self, tmp_path):
        onedir = tmp_path / "vox-server-rocm"
        _write(onedir / "vox-server-rocm.exe")
        ...
        server = out / "vox-server-rocm.tar.gz"
        libs = out / "rocm-libs-rocm7.2-v1.tar.gz"
        assert server.exists()
        assert libs.exists()
        assert (out / "vox-server-rocm.tar.gz.sha256").exists()
        ...
        assert "vox-server-rocm.exe" in core_names
