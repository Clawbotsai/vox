"""Vox MCP tool implementations.

Thin wrappers over existing services/routes. Tools are registered with dotted
names (``vox.speak`` etc.) so they look natural in agent logs —
...
def register_tools(mcp: FastMCP) -> None:
    """Attach all Vox tools to the given FastMCP instance."""

    @mcp.tool(
        name="vox.speak",
        description=(
            "Speak text in a Vox voice profile. Returns a generation id "
    ...
    async def vox_transcribe(
        ...
        # to loopback callers so a Vox bound on 0.0.0.0 doesn't double
    ...
    @mcp.tool(
        name="vox.list_profiles",
        description=(
            "List available voice profiles (both cloned voices and presets). "
            "Use the returned `name` with vox.speak(profile=...)."
        ),
    )
    async def vox_list_profiles() -> dict[str, Any]:
        db = next(get_db())
        try:
...
async def _transcribe_file(
    ...
    if (
        ...
        raise ValueError(
            ...
            "Vox → Settings → Models to download it first."
        )
