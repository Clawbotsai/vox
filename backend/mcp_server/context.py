...
MCP clients identify themselves via an ``X-Vox-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``VOX_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see vox.transcribe).
...
# Endpoints that consume X-Vox-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (vox.speak, vox.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of vox.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-Vox-Client-Id into a ContextVar and stamp last_seen_at
