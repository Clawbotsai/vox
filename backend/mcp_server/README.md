...
The server runs inside the same `uvicorn` process as the rest of Vox
...
`vox-mcp` binary ships inside the Vox.app bundle:
...
| Name | Purpose |
|---|---|
| `vox.speak`          | Speak text in a voice profile. Returns a generation id you can poll. |
| `vox.transcribe`     | Whisper transcription of a base64 blob or an absolute local path. |
| `vox.list_captures`  | Recent captures (dictation / recording / file) with transcripts. |
...
2. Per-client binding keyed by `X-Vox-Client-Id`
...
Point it at the URL, hit "List tools," call `vox.list_profiles`
first to confirm wiring, then `vox.speak` for end-to-end.
