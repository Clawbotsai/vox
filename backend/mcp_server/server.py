...
directly via URL; older stdio-only clients use the ``vox-mcp`` shim
...
def build_mcp_server() -> FastMCP:
    """Create the FastMCP instance with Vox tools registered."""
    mcp = FastMCP(
        name="vox",
        instructions=(
            "Vox is a local voice I/O layer. Use `vox.speak` to "
            "play text in a voice profile, `vox.transcribe` for "
...
def compose_lifespan(*lifespans):
    ...
    Used by ``create_app`` to run the existing Vox startup/shutdown
