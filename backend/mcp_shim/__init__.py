"""Stdio → Streamable HTTP bridge for the Vox MCP server.
...
the client spawns us as ``vox-mcp``; we proxy every JSON-RPC frame
to http://127.0.0.1:17493/mcp/ and stream responses back out.

All the real work (tools, models, inference) lives in the Vox server
