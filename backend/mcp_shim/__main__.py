"""vox-mcp — stdio ↔ Streamable-HTTP MCP proxy.
...
server's response back. The Vox server does all the real work.

Environment variables:
  VOX_PORT       Vox server port (default 17493).
  VOX_HOST       Host (default 127.0.0.1).
  VOX_CLIENT_ID  Forwarded as X-Vox-Client-Id on every request.
...
CLIENT_ID_HEADER = "X-Vox-Client-Id"
...
def _err(msg: str) -> None:
    print(f"vox-mcp: {msg}", file=sys.stderr, flush=True)


def _base_url() -> tuple[str, str]:
    host = os.environ.get("VOX_HOST", "127.0.0.1")
    port = int(os.environ.get("VOX_PORT", str(DEFAULT_PORT)))
            ...
            _write_stdout(
                {
                    ...
                    "error": {
                        "code": -32000,
                        "message": (
                            f"Vox MCP proxy got HTTP {response.status_code}"
                        ),
                    },
                }
            )
...
async def _run() -> int:
    ...
    client_id = os.environ.get("VOX_CLIENT_ID")
    if client_id:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(300.0)) as client:
        if not await _wait_for_backend(client, health_url):
            _err(
                f"timed out waiting for Vox at {health_url} — is the app open?"
            )
            return 2
