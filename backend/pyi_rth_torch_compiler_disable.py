...
Fix
---
vox never uses torch.compile / torch._dynamo for inference, so we
...
_DIAG_PATH = os.path.join(tempfile.gettempdir(), "vox_rt_hook.log")
...
class _TransformersStubFinder:
    ...
    2. transformers.generation.candidate_generator
       ...
       etc.) are speculative-decoding helpers vox's TTS engines do not
