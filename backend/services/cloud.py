"""
Vox Cloud device login — the "Log in with browser" flow.
...
callback. We exchange that code (server-to-server, over TLS) for a ``vox_…``
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if payload is None:
                ...
                return False, "Vox Cloud returned an unexpected response."
            api_key = payload.get("key")
            device_name = payload.get("label")
            if not api_key:
                return False, "Vox Cloud did not return a key."
        ...
        return False, "Could not reach Vox Cloud. Check your connection and try again."

    _store_key(db, api_key=api_key, device_name=device_name, account_user_id=account_user_id)
    logger.info("connected to Vox Cloud as device %r", device_name)
...
def get_status(db: Session) -> dict:
    ...
    # Prefix only: "vox_" (9) + 8 chars, matching the cloud's key_prefix.
