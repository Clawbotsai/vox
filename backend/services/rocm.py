...
Downloads two archives from GitHub Releases:
  1. Server core (vox-server-rocm.tar.gz) — the exe + non-AMD deps,
...
def get_rocm_exe_name() -> str:
    ...
    if sys.platform == "win32":
        return "vox-server-rocm.exe"
    return "vox-server-rocm"
...
def is_rocm_active() -> bool:
    ...
    """
    return os.environ.get("VOX_BACKEND_VARIANT") == "rocm"
...
async def _download_rocm_binary_locked(version: Optional[str] = None):
    ...
    server_archive = "vox-server-rocm.tar.gz"
