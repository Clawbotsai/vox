...
Downloads two archives from GitHub Releases:
  1. Server core (vox-server-cuda.tar.gz) — the exe + non-NVIDIA deps,
...
GITHUB_RELEASES_URL = "https://github.com/Clawbotsai/vox/releases/download"
...
def get_cuda_exe_name() -> str:
    ...
    if sys.platform == "win32":
        return "vox-server-cuda.exe"
    return "vox-server-cuda"
...
def is_cuda_active() -> bool:
    ...
    """
    return os.environ.get("VOX_BACKEND_VARIANT") == "cuda"
...
async def _download_cuda_binary_locked(version: Optional[str] = None):
    ...
    server_archive = "vox-server-cuda.tar.gz"
...
def get_cuda_binary_version() -> Optional[str]:
    ...
    try:
        ...
        # Output format: "vox-server 0.3.0"
        for line in result.stdout.strip().splitlines():
            if "vox-server" in line:
                return line.split()[-1]
    except Exception as e:
