"""
Entry point for PyInstaller-bundled vox server.
...
if "--version" in sys.argv:
    ...
    print(f"vox-server {__version__}")
    sys.exit(0)
...
if re.search(r"vox-server-rocm(\.exe)?$", _binary_name):
    os.environ["VOX_BACKEND_VARIANT"] = "rocm"
elif re.search(r"vox-server-cuda(\.exe)?$", _binary_name):
    os.environ["VOX_BACKEND_VARIANT"] = "cuda"
else:
    os.environ.setdefault("VOX_BACKEND_VARIANT", "cpu")
...
logger.info("=" * 60)
logger.info("vox-server starting up...")
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(description="vox backend server")
        parser.add_argument(
            "--host",
            type=str,
            default="127.0.0.1",
        ...
        logger.info(f"Backend variant: {os.environ.get('VOX_BACKEND_VARIANT', 'cpu').upper()}")
