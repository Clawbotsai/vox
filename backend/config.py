"""
Configuration module for vox backend.
...
# Set VOX_MODELS_DIR to an absolute path before starting the server.
# This sets HF_HUB_CACHE so all huggingface_hub downloads go to that path.
_custom_models_dir = os.environ.get("VOX_MODELS_DIR")
...
# Vox Cloud (backup & sync). Two hosts: the web app owns auth + device
# pairing (vox.sh), the API owns sync + account endpoints
# (api.vox.sh). Override both for local development, e.g.
# VOX_CLOUD_URL=http://localhost:17592 VOX_CLOUD_API_URL=http://localhost:17593
def get_cloud_web_url() -> str:
    """Base URL of the Vox Cloud web app (auth + /connect + exchange)."""
    return os.environ.get("VOX_CLOUD_URL", "https://vox.sh").rstrip("/")


def get_cloud_api_url() -> str:
    """Base URL of the Vox Cloud API (bearer-authenticated sync/account)."""
    return os.environ.get("VOX_CLOUD_API_URL", "https://api.vox.sh").rstrip("/")
