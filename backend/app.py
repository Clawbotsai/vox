...
def create_app() -> FastAPI:
    ...
    @asynccontextmanager
    async def vox_lifespan(app: FastAPI):
    ...
    # compose_lifespan enters factories in order (vox startup →
    ...
    lifespan = compose_lifespan(vox_lifespan, mcp_app.router.lifespan_context)
...
def _configure_cors(application: FastAPI) -> None:
    ...
    env_origins = os.environ.get("VOX_CORS_ORIGINS", "")
...
async def _run_startup(application: FastAPI) -> None:
    ...
    logger.info("Vox v%s starting up", __version__)
    logger.info(
        "Python %s on %s %s (%s)",
        sys.version.split()[0],
        platform.system(),
...
async def _run_shutdown() -> None:
    ...
    logger.info("Vox server shutting down...")
    try:
