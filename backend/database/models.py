"""ORM model definitions for the vox SQLite database."""
...
class CaptureSettings(Base):
    ...
    # "Vox would like to receive keystrokes from any application" dialog
...
class CloudSettings(Base):
    """Singleton row holding the link to a Vox Cloud account.
    ...
    api.vox.sh — auth only, never an encryption key (E2E key material lives
...
class MCPClientBinding(Base):
    ...
    itself via the ``X-Vox-Client-Id`` HTTP header; direct-HTTP
    clients set it in their MCP config's ``headers`` block, the stdio
    shim forwards it from the ``VOX_CLIENT_ID`` env var.
    """
    ...
    # When true, vox.speak routes through the profile's personality LLM
