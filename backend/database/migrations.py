"""Column-level migrations for the vox SQLite database.

Why not Alembic?  vox is a single-user desktop app shipping as a
...
def _migrate_mcp_bindings(engine, inspector, tables: set[str]) -> None:
    ...
    to a boolean: when true, ``vox.speak`` rewrites input through the
