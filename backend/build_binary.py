...
def build_server(cuda=False, rocm=False):
    ...
    Args:
        cuda: If True, build with CUDA support and name the binary
              vox-server-cuda instead of vox-server.
        rocm: If True, build with ROCm support and name the binary
              vox-server-rocm instead of vox-server.
    """
    if cuda and rocm:
    ...
    if rocm:
        binary_name = "vox-server-rocm"
    elif cuda:
        binary_name = "vox-server-cuda"
    else:
        binary_name = "vox-server"
    ...
    args.extend(
        [
            ...
            "--hidden-import",
            "loguru",
            # MCP server — Streamable-HTTP endpoint and the 4 vox.* tools.
...
def build_shim():
    """Build the vox-mcp stdio shim as a tiny standalone binary.

    This is the bridge for MCP clients that only speak stdio — it proxies
    JSON-RPC to the main vox-server's /mcp endpoint. Keep it small: no
    ...
    os.chdir(backend_dir)
    PyInstaller.__main__.run(args)
    logger.info("Shim built: %s", backend_dir / "dist" / "vox-mcp")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build vox binaries")
    parser.add_argument(
        "--cuda",
        action="store_true",
        help="Build CUDA-enabled binary (vox-server-cuda)",
    )
    parser.add_argument(
        "--rocm",
        action="store_true",
        help="Build ROCm-enabled binary (vox-server-rocm) for AMD GPUs",
    )
    parser.add_argument(
        "--shim",
        action="store_true",
        help="Build the vox-mcp stdio shim binary instead of the server",
    )
